...
const SubTask = ({ index, task, setTask }) => {
    ...
    return (
        ...
        <Box className='flex w-full items-center gap-2'>
            <Checkbox icon={<CircleOutlined />} checkedIcon={<Circle />} value={checked} onChange={(e) => { setTask(state => ({ ...state, subtasks: state.subtasks.map((el, i) => ((i === index) ? { ...state.subtasks[i], checked: !checked } : el)) })) }} />
